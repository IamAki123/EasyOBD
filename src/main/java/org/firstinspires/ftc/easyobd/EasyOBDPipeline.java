 ...
 * ball-center plane inside {@link EasyOBDConfig#maxRangeInches}.
 ...
 *
 * <h2>Resolution</h2>
 * Stream at 640×480 for preview; set {@link EasyOBDConfig#processWidth} to
 ...
 */
public class EasyOBDPipeline extends OpenCvPipeline {
    ...
    // ---- Defaults (copied into a new pipeline; prefer EasyOBDConfig). ----
    public static int GRID_ROWS = EasyOBDConfig.DEFAULT_GRID_ROWS;
    public static int GRID_COLS = EasyOBDConfig.DEFAULT_GRID_COLS;
    public static double WHITE_RATIO_THRESHOLD = EasyOBDConfig.DEFAULT_WHITE_RATIO_THRESHOLD;
    public static double BALL_SEARCH_RADIUS_PX = EasyOBDConfig.DEFAULT_BALL_SEARCH_RADIUS_PX;
    public static Scalar yellowLower = new Scalar(
            EasyOBDConfig.DEFAULT_H_LOWER, EasyOBDConfig.DEFAULT_S_LOWER, EasyOBDConfig.DEFAULT_V_LOWER);
    public static Scalar yellowUpper = new Scalar(
            EasyOBDConfig.DEFAULT_H_UPPER, EasyOBDConfig.DEFAULT_S_UPPER, EasyOBDConfig.DEFAULT_V_UPPER);
    public static double MIN_BALL_DIAMETER_INCHES = EasyOBDConfig.DEFAULT_MIN_BALL_DIAMETER_INCHES;
    public static double MAX_RANGE_INCHES = EasyOBDConfig.DEFAULT_MAX_RANGE_INCHES;
    public static double HORIZONTAL_FOV_DEGREES = EasyOBDConfig.DEFAULT_HORIZONTAL_FOV_DEGREES;
    public static double FOCAL_LENGTH_PIXELS_AT_640 = EasyOBDConfig.DEFAULT_FOCAL_LENGTH_PIXELS_AT_640;
    public static double CAMERA_HEIGHT_INCHES = EasyOBDConfig.DEFAULT_CAMERA_HEIGHT_INCHES;
    public static double CAMERA_TILT_DEGREES = EasyOBDConfig.DEFAULT_CAMERA_TILT_DEGREES;
    public static double CAMERA_FORWARD_OF_CENTER = EasyOBDConfig.DEFAULT_CAMERA_FORWARD_OF_CENTER;
    public static double CAMERA_RIGHT_OF_CENTER = EasyOBDConfig.DEFAULT_CAMERA_RIGHT_OF_CENTER;
    public static double CAMERA_YAW_RAD = Math.toRadians(EasyOBDConfig.DEFAULT_CAMERA_YAW_DEGREES);
    public static double PROCESS_SCALE = EasyOBDConfig.DEFAULT_PROCESS_SCALE;

    private final EasyOBDConfig cfg;
    ...
    public EasyOBDPipeline() {
        this(EasyOBDConfig.fromPipelineStatics());
    }

    public EasyOBDPipeline(EasyOBDConfig config) {
        this.cfg = config == null ? EasyOBDConfig.defaults() : config;
    }

    /** Live-tunable config for this pipeline (not shared with other instances). */
    public EasyOBDConfig getConfig() {
        return cfg;
    }

    public void setConfig(EasyOBDConfig replacement) {
        if (replacement == null) {
            return;
        }
        ...
        cfg.yellowLower = replacement.yellowLower.clone();
        cfg.yellowUpper = replacement.yellowUpper.clone();
        cfg.extraColorRanges.clear();
        for (EasyOBDConfig.ColorRange range : replacement.extraColorRanges) {
    ...
    public static void nudgeHsvRange(int delta) {
        ...
        yellowLower.val[0] = EasyOBDConfig.clamp(yellowLower.val[0] - delta, 0, 28);
        yellowUpper.val[0] = EasyOBDConfig.clamp(yellowUpper.val[0] + delta, 24, 50);
        yellowLower.val[1] = EasyOBDConfig.clamp(yellowLower.val[1] - 5 * delta, 20, 200);
        yellowLower.val[2] = EasyOBDConfig.clamp(yellowLower.val[2] - 5 * delta, 20, 200);
    }
    ...
    private void applyColorMask(Mat hsv, Mat dst) {
        ...
        Core.inRange(hsv, lower, cfg.yellowUpper, dst);
        for (EasyOBDConfig.ColorRange range : cfg.extraColorRanges) {
    ...
    private List<ClusterInfo> publishAllClusters(ClusterResult grid, List<Ball> found,
        ...
        for (StateGroup group : grid.groups) {
            ...
            double confidence = EasyOBDConfig.clamp(circ, 0, 1);
            if (near.isEmpty()) {
                confidence = 0.45;
            }
    ...
    private Ball makeBall(Point center, double radiusPx, double area, double circularity,
        ...
        double confidence = EasyOBDConfig.clamp(circularity * Math.min(1.0, fillRatio / 0.85), 0, 1);
